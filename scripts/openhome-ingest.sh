...
build_index() {
...

  {
    ...
    for section in platform abilities builders concepts roadmap; do
...
case "$ENTITY_TYPE" in
  ...
  query)
    ...
    ;;
  */*)
    # Single page: e.g. openhome-ingest platform/speakers
    ingest_page "$ENTITY_TYPE"
    ;;
  *)
    echo "Usage: openhome-ingest [all|platform|abilities|builders|concepts|roadmap|index|lint|query|<entity/page>]" >&2
    exit 1
    ;;
esac
